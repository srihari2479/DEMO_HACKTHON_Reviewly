-- 1. Drop the old table to clean up all existing duplicates
DROP TABLE IF EXISTS pr_audits CASCADE;

-- 2. Create the table with the UNIQUE constraint built-in including user_email
CREATE TABLE pr_audits (
  ...
  user_email VARCHAR(255) NOT NULL, -- Added to track user-based audits
  CONSTRAINT unique_pr_repo UNIQUE (user_email, repository, pr_number)
);

-- 3. Seed the initial mock data safely using the conflict check (defaulted to standard user email)
INSERT INTO pr_audits (pr_number, title, author, repository, status, git_diff, before_screenshot_url, after_screenshot_url, ai_summary, ai_risks, user_email)
VALUES (
  104,
...
+      <a href="/signup" className="text-gray-500 mt-8">Don''t have an account? Sign Up</a>',
  ...
  'https://images.unsplash.com/photo-1618005198143-e528346d9a59?w=600&auto=format&fit=crop&q=60',
  ...
  'demo@reviewly.ai'
) ON CONFLICT (user_email, repository, pr_number) DO NOTHING;
